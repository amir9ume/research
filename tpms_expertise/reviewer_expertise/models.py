...
class Flip_Attention_Module(nn.Module):
    def __init__(self, num_topics, attention_matrix_size, dropout):
        super(Flip_Attention_Module, self).__init__()
        self.num_topics = num_topics
        self.attention_matrix_size = attention_matrix_size
        self.W_Q = nn.Linear(self.num_topics, self.attention_matrix_size)
        self.max_length = 350
        self.W_K = nn.Linear(self.max_length, self.attention_matrix_size)
        self.W_V = nn.Linear(self.num_topics, self.attention_matrix_size)
        self.outer = nn.Linear(350, 25)
        self.padding = True
    ...
    def forward(self, submitter_emb, reviewer_emb):
        Q = self.W_Q(submitter_emb)
        reviewer_emb = reviewer_emb.permute(0, 2, 1)
        K = self.W_K(reviewer_emb)
        normalizing_factor = (math.sqrt(self.attention_matrix_size))
        e = (torch.bmm(Q.unsqueeze(dim=1), K.permute(0, 2, 1)) / normalizing_factor)
        e = F.softmax(e, dim=2)
        ...
        ww = self.outer(ww)
        ww = nn.Tanh()(ww)
    #    ww=torch.sum(ww,dim=1).unsqueeze(dim=1)
        return ww
...
class Match_LR(nn.Module):
    def __init__(self, batch_size, submitter_emb_dim, reviewer_emb_dim,
        ...
        self.weights_add = Variable(torch.Tensor(
            submitter_emb_dim), requires_grad=True).to(device) 
        ...
        if self.attn_over_docs:
            ...
            if self.number_heads == 1:
                ...
                # self.attention_module = Flip_Attention_Module(
                #     25, 20, self.dropout)
            else:
                self.attention_module = Multi_Head_Attention(
                    self.number_heads, 25, 20, self.dropout)
            self.init_weights()
    ...
    def get_distance_distributions


    def get_reviewer_weighted(self, submitter_emb, reviewer_emb):
        weighted_reviewer_emb = self.attention_module(
    ...
    def forward(self, submitter_emb, reviewer_emb):
        ...
        # + nn.Tanh()(self.weights_multi * multi))
        combo = self.combined(
...
class Regression_Attention_Over_docs(nn.Module):
    def __init__(self,
        ...
        self.w_submitted = nn.Linear(
            self.num_topics, self.attention_matrix_size)
        self.w_out = nn.Linear(self.num_topics*2, 1)
        if self.attn_flag:
            self.attention_module = Attention_Module(
                self.num_topics, self.attention_matrix_size, self.dropout)
    ...
    def forward(self, submitter_emb, reviewer_emb):
        if self.attn_flag:
            ...
            combine = torch.cat(
                (self.W_V(x), self.w_submitted(submitter_emb)), dim=1)
